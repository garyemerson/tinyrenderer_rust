...
fn run_model_with_zbuffer_and_texture() {
    let (width, height): (f32, f32) = (800.0, 800.0);
    let mut img = Img::new(width as u32, height as u32);
...
fn run_model_with_zbuffer_texture_perspective() {
    let (width, height): (f32, f32) = (800.0, 800.0);
    let mut img = Img::new(width as u32, height as u32);
